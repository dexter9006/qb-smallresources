...
RegisterCommand(Config.HandsUp.command, function()
    ...
    handsUp = not handsUp
    if exports['qb-policejob']:IsHandcuffed() or PlayerData.metadata["isdead"] or PlayerData.metadata["inlaststand"] then return end --vitto add multiple states
    if handsUp then
        TaskPlayAnim(ped, 'missminuteman_1ig_2', 'handsup_base', 8.0, 8.0, -1, 50, 0, false, false, false)
        --TaskPlayAnim(ped, 'missminuteman_1ig_2', 'handsup_base', 2.0, 2.5, -1, 50, 0, false, false, false) -- c'est ce qu'on avait dans la backup
